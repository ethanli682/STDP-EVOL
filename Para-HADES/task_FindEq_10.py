...
def run_task(param, args):
    ...
    def complex_equation_solution(vector):
        if len(vector) != 10:
            raise ValueError("Input vector must contain exactly 10 numbers.")

        # Use the vector values as variables a, b, c, d, e, f, g, h, i, j
        a, b, c, d, e, f, g, h, i, j = vector

        # Define the complex equation (example: a quadratic equation, arbitrary)
        proposed_solution = (a**2 + b**3 - c / (d + 1) + e * np.sin(f) +
                            g**2 * np.log(h + 1) - i**3 + j)

        return proposed_solution


    global target
    target = list([42, 3.14, 2.71, 666, -273.15])    
    ...
    # Get the proposed solution from the complex equation
    proposed_solution = complex_equation_solution(source)

    for tar in target:
        actual_solution = tar
        # Calculate the difference (closeness value)
        closeness_value = np.abs(proposed_solution - actual_solution)
        outP['tests'].append(float(str(closeness_value)))
...
def main(args):
    ...
    try:
        plotter_config = {
            ...
            'better_direction': 'lower',          # Lower error is better
            'target_labels': [str(t) for t in target],  # Show target values
            'enable_clustering': True,            # Enable clustering visualization
            'enable_3d_viz': False,               # Don't need 3D for this task
            'enable_individual_targets': False,   # Clean output without individual plots
            'plot_frequency': 0.1,                # 10% chance to plot (performance)
            'main_plot_title': 'Convergence to Equation Targets',
            'dpi': 300
        }
        
        plotter = UnifiedPlotter(args, targets=[[t] for t in target], config=plotter_config)
