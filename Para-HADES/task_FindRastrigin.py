...
def run_task(param, args):
    ...
    # 1. Look for 'Rastrigin' parameter
    if 'Rastrigin' in param['param']:
        source = param['param']['Rastrigin']
    else:
        # Graceful failure if the key isn't found
        print("Error: Parameter 'Rastrigin' not found in input.", flush=True)
        return

    # 2. Infer dimension from input list size
    # This adapts the code to fit ANY dimension (2, 4, 32, 256, or anything else)
    x = np.array(source, dtype=float).squeeze()
    
    # Validation: Ensure input is not empty
    if x.size == 0:
        print("Error: Input vector 'Rastrigin' is empty.", flush=True)
        return

    d = x.size  # Automatically detected dimension
    
    # Optional: Log the detected dimension for verification
    # print(f"Detected Rastrigin Dimension: {d}", flush=True)

    # 3. Calculate Rastrigin Value (Scalable to any 'd')
    # Formula: 10*d + sum(x^2 - 10cos(2*pi*x))
    # Domain is usually [-5.12, 5.12]
    A = 10
    
    val = A * d + np.sum(x**2 - A * np.cos(2 * np.pi * x))
    
    outP['tests'].append(val)

    # 4. Set Fitness Score
    # Rastrigin is a MINIMIZATION problem (we want val -> 0).
    # If evolutionTarget == -1 (Minimize mode), return val.
    # If evolutionTarget != -1 (Maximize mode), return -val.
    
    if args.evolutionTarget == -1:
        outP['fitnessScore'] = float(val)
    else:
        outP['fitnessScore'] = -float(val)
