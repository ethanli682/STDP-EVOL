...
def _run_task_impl(param, args):
    ...
    # =========================================================================
    # Static MLP (non-plastic)  –  CoGames tutorial.scout
    # =========================================================================
    # Topology identical to MLP_heb (no bias, no plasticity):
    #   900 -> h1 -> h2 -> 5   (defaults h1=128, h2=64)
    # The evolved gene supplies three flat weight arrays:
    #   param['param']['W1']  shape (h1 * 900,)
    #   param['param']['W2']  shape (h2 * h1,)
    #   param['param']['W3']  shape (5  * h2,)
    # =========================================================================
    ...
    STATIC_EVAL_SCRIPT = os.path.join(task_dir, 'evaluate_static_cogames.py')
    ...
    hidden             = (128, 64)
    # Observation pipeline defaults — overridable via the StaticEval YAML block.
    obs_mode           = 'legacy_flat'   # 'legacy_flat' | 'stacked_grid'
    frame_stack        = 2               # only used when obs_mode == 'stacked_grid'
    obs_encoder        = 'none'          # 'none' | 'random_proj'
    encoder_dim        = 64
    encoder_seed       = 1337
    fitness_v4_weights_str = ''   # comma-separated key=val list passed to evaluator
    ...
    try:
        if isinstance(args.paramFile, str) and os.path.exists(args.paramFile):
            ...
            if 'seed_base' in _cfg and _cfg['seed_base'] is not None:
                # Common-random-seeds across the population in a single generation:
                # within one epoch, every candidate sees the same map seeds, which
                # crisply isolates weight quality from map luck. Between epochs
                # the seed rotates so the population does not overfit to one map.
            ...
            if isinstance(_fv5, dict):
                # Allow string values for the special 'reduction' key; everything
                # else is coerced to float.
                _parts = []
    ...
    h1, h2 = hidden
    ACTION_DIM  = 5
    # Compute INPUT_DIM symmetrically to evaluate_static_cogames._resolve_input_dim
    # so the gene-shape sanity check matches what the evaluator actually wires
    # into MLP_static_cogames.fc1.
    _PER_FRAME_RAW = 433 if obs_mode == 'stacked_grid' else 900   # 11*11*3 + 70
    ...
    EXPECTED = {'W1': h1 * INPUT_DIM, 'W2': h2 * h1, 'W3': ACTION_DIM * h2}
    print(
        f'Static task: obs_mode={obs_mode} frame_stack={frame_stack} '
    ...
    # Note: seed is intentionally NOT a function of agent_idx or wall-clock/pid —
    # all candidates in this generation share map seeds (common-random-seeds),
    # so fitness differences reflect weight differences, not map luck.
    print(f'Effective seed_base={seed_base} (agent_idx={_agent_idx})  hidden={hidden}', flush=True)
    ...
    W1 = _get_array('W1', EXPECTED['W1'])
    W2 = _get_array('W2', EXPECTED['W2'])
    W3 = _get_array('W3', EXPECTED['W3'])
    _debug_stage(args, 'task_impl.gene_arrays_loaded')
    print(
        f"Static weights loaded: "
        f"W1∈[{W1.min():.3f}, {W1.max():.3f}]  "
    ...
    weights_path = os.path.join(
        taskPath,
        f'static_weights_{args.agent_idx}_{args.agent_test_num}_{unique_suffix}.dat'
    )
    ...
    # --- Build and launch evaluate_static_cogames.py -------------------------
    cmd = [
    ...
    print(f'evaluate_static_script={STATIC_EVAL_SCRIPT}', flush=True)
    ...
    # Read v5 components sidecar before cleaning it up (only present under
    # composite_v5_role_event).
    components_v5 = None
    ...
    # Same for v7 (only present under composite_v7_zstat).
    components_v7 = None
