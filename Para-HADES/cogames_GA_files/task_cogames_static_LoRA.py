...
def _run_task_impl(param, args):
    ...
    # =========================================================================
    # Static LoRA MLP (non-plastic)  –  CoGames tutorial.scout
    # =========================================================================
    # For each layer l ∈ {1,2,3}:
    #   W_l = beta_l * W0_l + U_l @ V_l
    # W0_l is frozen, deterministic gaussian noise (seeded by main.StaticLoRA.w0_seed),
    # with sigma = 1/sqrt(d_in). Rank r defaults to 8.
    # Layer shapes:
    #   l1: (900,128)  U:(900,r)  V:(r,128)
    #   l2: (128,64)   U:(128,r)  V:(r,64)
    #   l3: (64,5)     U:(64,r)   V:(r,5)
    # =========================================================================
    ...
    if obs_mode == 'stacked_grid':
        ...
        INPUT_DIM = 900
    ACTION_DIM = 5
    LAYER_SIZES = [(INPUT_DIM, h1), (h1, h2), (h2, ACTION_DIM)]

    _agent_idx = int(getattr(args, 'agent_idx', 0))
    # seed_base is shared across all candidates in this generation; see comment above.
    print(
        f'Effective seed_base={seed_base} (agent_idx={_agent_idx})  hidden={hidden}  '
        f'rank={lora_rank}  w0_seed={w0_seed}  '
        f'obs_mode={obs_mode}  frame_stack={frame_stack}  input_dim={INPUT_DIM}',
        flush=True,
    )

    # --- Extract LoRA factors from gene and compose dense weights ----------
    ...
    def _get_beta(layer_num):
        key = f'beta_l{layer_num}'
    ...
    def _get_uv(layer_num, d_in, d_out):
        uk, vk = f'W_l{layer_num}_u', f'W_l{layer_num}_v'
        if uk not in gene_dict or vk not in gene_dict:
            raise ValueError(f"Missing LoRA gene keys '{uk}'/'{vk}'.")
    ...
    for layer_idx, (d_in, d_out) in enumerate(LAYER_SIZES):
        ...
        beta_val = _get_beta(layer_num)
        u_mat, v_mat = _get_uv(layer_num, d_in, d_out)
        layer_rng = np.random.default_rng(int(w0_seed + layer_idx))
        sigma_w0  = 1.0 / np.sqrt(float(d_in))
        w0 = layer_rng.normal(0.0, sigma_w0, size=(d_in, d_out)).astype(np.float32)
        dense = (beta_val * w0 + np.matmul(u_mat, v_mat)).astype(np.float32)
        # nn.Linear expects (out, in), so transpose (d_in, d_out) -> (d_out, d_in)
        composed.append(dense.T.reshape(-1))

    W1, W2, W3 = composed[0], composed[1], composed[2]
