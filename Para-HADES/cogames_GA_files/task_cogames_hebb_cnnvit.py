...
def run_task(param, args):
    ...
    try:
        ...
        # CNN+ViT runs once per agent per step, so this variant is much heavier
        # per rollout than the flat-projection Hebbian. Widen the worker timeout
        # (was 930) to keep the inner eval (timeout below) from being killed by
        # its wrapper; both stay well under the SLURM Task_resource wall time.
        return_code, stdout, stderr = _run_subprocess_interruptible(cmd, timeout=1500)
...
def _run_task_impl(param, args):
    ...
    HEBB_EVAL_SCRIPT = os.path.join(task_dir, 'evaluate_hebb_cnnvit_cogames.py')
    HEBB_RULE        = 'ABCD_lr'
    ...
    # This CNN+ViT variant rasterises tokens into a (num_features,H,W) image and
    # runs a FROZEN CNN+ViT that projects to encoder_dim before the Hebbian MLP.
    obs_encoder      = 'cnn_vit'   # frozen CNN+ViT front end (see evaluate_hebb_cnnvit_cogames.py)
    encoder_dim      = 64
    encoder_seed     = 1337
    ...
    if obs_encoder in ('random_proj', 'cnn_vit'):
    ...
    try:
        ...
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=1200,   # CNN+ViT is heavier per step than flat-proj (was 600)
        )
