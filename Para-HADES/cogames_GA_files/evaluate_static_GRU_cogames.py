...
# ---------------------------------------------------------------------------
# Two modes are supported via --obs_mode:
#
#   legacy_flat  : flatten raw (num_tokens, token_dim) uint8 array / 255.
#                  Input dim = num_tokens * token_dim (= 900 for tutorial.scout).
#
#   stacked_grid : decode to 11x11x3 spatial grid + length-70 global vector
#                  per frame (433 dims), frame-stack the last FRAME_STACK
#                  frames oldest->newest.
#
# An optional frozen random projection (see `obs_encoder.py`) collapses each
# raw frame to `encoder_dim` floats before stacking and before the network's
# input weight matrix sees it. This is the lever that shrinks W1 enough for
# CMA-ES to make progress at population-feasible sizes.
# ---------------------------------------------------------------------------
...
def legacy_obs_to_flat(tokens, obs_shape) -> np.ndarray:
    """Original token-array-flattened encoding (kept for backward compat)."""
...
# ---------------------------------------------------------------------------
# Static GRU policy: projection -> GRUCell -> linear. No bias, no plasticity.
# Hidden state is carried across timesteps within an episode and reset at the
# start of each new episode by creating a fresh CoGamesGRUAgentPolicy instance.
# ---------------------------------------------------------------------------

class GRU_static_cogames(nn.Module):
    def __init__(self, input_dim: int, action_dim: int, proj: int = 64, hidden: int = 32):
        super().__init__()
        self.hidden_size = hidden
        self.proj = nn.Linear(input_dim, proj, bias=False)
        self.gru  = nn.GRUCell(proj, hidden, bias=False)
        self.out  = nn.Linear(hidden, action_dim, bias=False)

    def init_hidden(self) -> torch.Tensor:
        return torch.zeros(1, self.hidden_size)

    def forward(self, obs_flat: np.ndarray, h: torch.Tensor):
        x = torch.as_tensor(obs_flat).float().detach().unsqueeze(0)
        x = torch.tanh(self.proj(x))
        h = self.gru(x, h)
        return self.out(h).squeeze(0), h


class CoGamesGRUAgentPolicy(AgentPolicy):
    def __init__(
        self,
        policy_env_info: PolicyEnvInterface,
        network: GRU_static_cogames,
    ...
    def step(self, obs: AgentObservation) -> Action:
        ...
        with torch.no_grad():
            logits, self._h = self._p(net_in, self._h)
    ...
    if fitness_stat == 'composite_exploration':
        ...
        for s in agent_stats_list:
            ...
            agent_scores.append(
                ...
                + 0.4 * efficiency
                + 0.3 * dist / dist_norm
                - 0.5 * fail_ratio ** 2     # convex wall-hit penalty (matches evaluate_static_cogames.py)
            )
        return float(np.mean(agent_scores))

    if fitness_stat == 'composite_exploration_v2':
        # See evaluate_static_cogames.py for the rationale. Summary: v1 rewarded
        # "stand still" at score ≈ 0 while punishing any active policy via the
        # convex wall penalty, which sank optimisation into a do-nothing basin.
        agent_stats_list = sim.episode_stats['agent']
        max_steps = max(env_cfg.game.max_steps or 1000, 100)
        dist_norm = max_steps ** 0.5
        ...
        for s in agent_stats_list:
            ...
            # Efficiency = fraction of moves reaching a cell beyond spawn; see
            # evaluate_static_cogames.py for the (unique - 1) rationale and
            # the note on mettagrid's 8x cell-ticks counter.
...
# ---------------------------------------------------------------------------
# Weight loading: flat arrays -> GRU_static_cogames
# ---------------------------------------------------------------------------

def _load_weights_into_model(model: GRU_static_cogames, weights_path: str,
                             input_dim: int, proj: int, hidden: int, num_actions: int) -> None:
    blob = torch.load(weights_path, weights_only=False)
    if not isinstance(blob, dict):
        raise ValueError('GRU weights file must contain a dict with keys W_in, W_ih, W_hh, W_out.')

    # nn.Linear.weight is (out, in); nn.GRUCell.weight_ih is (3*hidden, input).
    shapes = {
        'W_in':  (proj, input_dim),
        'W_ih':  (3 * hidden, proj),
        'W_hh':  (3 * hidden, hidden),
        'W_out': (num_actions, hidden),
    }
    targets = {
        'W_in':  model.proj.weight,
        'W_ih':  model.gru.weight_ih,
        'W_hh':  model.gru.weight_hh,
    ...
    for name, (out_dim, in_dim) in shapes.items():
        ...
        targets[name].data = torch.from_numpy(arr.reshape(out_dim, in_dim))
...
def evaluate_static_GRU_cogames(
    mission: str,
    weights_path: str,
    proj: int = 64,
    hidden: int = 32,
    ...
    with torch.no_grad():
        ...
        print(
            f'CoGames GRU eval: mission={mission}  obs_shape={obs_shape}  '
        ...
        for ep_idx in range(eval_episodes):
            ...
            for _agent_id in range(num_agents):
                net = GRU_static_cogames(input_dim, num_actions, proj=proj, hidden=hidden)
                _load_weights_into_model(net, weights_path, input_dim, proj, hidden, num_actions)
                agent_policies.append(
                    CoGamesGRUAgentPolicy(
                        policy_env_info, net,
...
def main(argv):
    parser = argparse.ArgumentParser(
        description='Evaluate a static GRU policy on CoGames.'
    )
