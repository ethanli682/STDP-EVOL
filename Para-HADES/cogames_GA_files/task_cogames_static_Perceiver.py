...
def _run_task_impl(param, args):
    ...
    # =========================================================================
    # Static Perceiver-lite cross-attention (non-plastic) – CoGames tutorial.scout
    # =========================================================================
    # Per-step tokens come from the stacked_grid encoding (frame_stack x 433).
    # A bank of `num_latents` persistent latents (dim d_model) is carried across
    # steps within an episode; each step the latents cross-attend to the frame
    # tokens and are refreshed via a residual tanh update. Mean-pooled latents
    # drive a linear head over actions.
    #
    # Evolved genes (flat arrays), with defaults (d_model=32, num_latents=8,
    # per_frame_dim=433, action_dim=5):
    #   W_proj : d_model * per_frame_dim     = 32 * 433 = 13,856
    #   W_q    : d_model * d_model           = 32 *  32 =  1,024
    #   W_k    : d_model * d_model           = 32 *  32 =  1,024
    #   W_v    : d_model * d_model           = 32 *  32 =  1,024
    #   W_o    : d_model * d_model           = 32 *  32 =  1,024
    #   L_init : num_latents * d_model       =  8 *  32 =    256
    #   W_out  : action_dim * d_model        =  5 *  32 =    160
    # TOTAL                                             = 18,368 evolved params.
    # =========================================================================
    ...
    STATIC_EVAL_SCRIPT = os.path.join(task_dir, 'evaluate_static_Perceiver_cogames.py')
    ...
    try:
        if isinstance(args.paramFile, str) and os.path.exists(args.paramFile):
            ...
            if 'seed_base' in _cfg and _cfg['seed_base'] is not None:
                # Common-random-seeds across all candidates in a generation.
            ...
            if 'd_model' in _cfg and _cfg['d_model'] is not None:
                d_model = int(_cfg['d_model'])
            if 'num_latents' in _cfg and _cfg['num_latents'] is not None:
                num_latents = int(_cfg['num_latents'])
    ...
    PER_FRAME_RAW = grid_h * grid_w * num_channels + global_state_dim
    if obs_encoder == 'random_proj':
        PER_FRAME_DIM = int(encoder_dim)
    else:
        PER_FRAME_DIM = PER_FRAME_RAW
    ACTION_DIM = 5
    EXPECTED = {
        ...
        'W_out':  ACTION_DIM * d_model,
    }

    _agent_idx = int(getattr(args, 'agent_idx', 0))
    print(
        f'Effective seed_base={seed_base} (agent_idx={_agent_idx})  '
        f'd_model={d_model}  num_latents={num_latents}  '
        f'frame_stack={frame_stack}  per_frame_raw={PER_FRAME_RAW}  '
        f'obs_encoder={obs_encoder}  encoder_dim={encoder_dim}  '
        f'per_frame_dim={PER_FRAME_DIM}  EXPECTED={EXPECTED}',
        flush=True,
    )
    ...
    W_proj = _get_array('W_proj', EXPECTED['W_proj'])
    W_q    = _get_array('W_q',    EXPECTED['W_q'])
    W_k    = _get_array('W_k',    EXPECTED['W_k'])
    W_v    = _get_array('W_v',    EXPECTED['W_v'])
    W_o    = _get_array('W_o',    EXPECTED['W_o'])
    L_init = _get_array('L_init', EXPECTED['L_init'])
    W_out  = _get_array('W_out',  EXPECTED['W_out'])
    _debug_stage(args, 'task_impl.gene_arrays_loaded')
    print(
        f"Perceiver weights loaded: "
        f"W_proj∈[{W_proj.min():.3f}, {W_proj.max():.3f}]  "
        f"W_q∈[{W_q.min():.3f}, {W_q.max():.3f}]  "
        f"W_k∈[{W_k.min():.3f}, {W_k.max():.3f}]  "
        f"W_v∈[{W_v.min():.3f}, {W_v.max():.3f}]  "
        f"W_o∈[{W_o.min():.3f}, {W_o.max():.3f}]  "
        f"L_init∈[{L_init.min():.3f}, {L_init.max():.3f}]  "
    ...
    weights_path = os.path.join(
        taskPath,
        f'perceiver_weights_{args.agent_idx}_{args.agent_test_num}_{unique_suffix}.dat'
    )
    torch.save(
        {
            'W_proj': W_proj, 'W_q': W_q, 'W_k': W_k, 'W_v': W_v,
            'W_o': W_o, 'L_init': L_init, 'W_out': W_out,
        },
        weights_path,
    )
    ...
    # --- Build and launch evaluate_static_Perceiver_cogames.py ---------------
    cmd = [
    ...
    print(f'evaluate_static_perceiver_script={STATIC_EVAL_SCRIPT}', flush=True)
