...
from cnn_vit_encoder import FrozenCNNViTEncoder


# ---------------------------------------------------------------------------
# CNN+ViT Hebbian variant.
#
# Byte-for-byte fork of evaluate_hebb_cogames.py with ONE change: the flat
# random-projection obs encoder is replaced by a FROZEN CNN + ViT that consumes
# a rasterised (C=num_features, H, W) egocentric image instead of the flattened
# 900-dim token vector. The image embedding (encoder_dim) then feeds the SAME
# plastic Hebbian MLP (encoder_dim -> 64 -> 32 -> actions), so the evolved
# genome (A/B/C/D/lr coefficients) is identical to the random_proj variant.
# See cnn_vit_encoder.py for the frozen front end.
# ---------------------------------------------------------------------------


# ---------------------------------------------------------------------------
# MLP architecture (mirrors policies.py:MLP_heb but with cogames dims)
# Hebbian MLP input = encoder_dim (CNN+ViT embedding), hidden=[64,32],
# output=num_actions
# ---------------------------------------------------------------------------
...
class CoGamesAgentPolicy(AgentPolicy):
    ...
    def __init__(
        ...
        # (C=num_features, H, W) egocentric image the tokens are rasterised into.
        assert img_shape is not None, 'CNN+ViT policy requires img_shape'
        self._img_shape = tuple(int(v) for v in img_shape)
    ...
    def _obs_to_image(self, obs: AgentObservation) -> np.ndarray:
        """Rasterise token observation into a (C, H, W) float32 image in [0,1].

        Each token carries a feature id, a value, and a spatial location within
        the egocentric window. We drop the value (normalised to [0,1] the same
        way _obs_to_flat did: /255) into channel=feature_id at cell (row, col).
        Global tokens (location None -- agent-wide state) are broadcast across
        their whole feature plane. Multiple tokens on one cell accumulate.
        """
        C, H, W = self._img_shape
        img = np.zeros((C, H, W), dtype=np.float32)
        for token in obs.tokens:
            fid = int(token.feature.id)
            if fid < 0 or fid >= C:
                continue
            val = float(token.value) / 255.0
            loc = token.location
            if loc is None:
                img[fid, :, :] += val
            else:
    ...
    def step(self, obs: AgentObservation) -> Action:
        # Frozen CNN+ViT front end: rasterise -> image -> encoder_dim embedding.
        obs_vec = self._encoder(self._obs_to_image(obs))

        # Forward pass
        with torch.no_grad():
            o0, o1, o2, o3 = self._p([obs_vec])
            o0_np = o0.numpy()
            o1_np = o1.numpy()
            o2_np = o2.numpy()
            o3_np = o3.numpy()
...
def evaluate_hebb_cogames(
    ...
    with torch.no_grad():
        ...
        # --- Frozen CNN+ViT front end -----------------------------------------
        # Rasterise tokens into a (C=num_features, H, W) egocentric image, then
        # collapse to encoder_dim with a FROZEN CNN+ViT. The Hebbian MLP input is
        # encoder_dim (identical genome to the random_proj variant). Image dims
        # are read from the env: C = number of observation features, H/W = the
        # obs window height/width.
        try:
            _num_features = len(env_cfg.game.id_map().features())
        except Exception:
            _num_features = 70   # cogs_vs_clips default (safe fallback)
        try:
            _obs_h = int(env_cfg.game.obs.height)
            _obs_w = int(env_cfg.game.obs.width)
        except Exception:
            _obs_h = _obs_w = 13
        img_shape = (int(_num_features), _obs_h, _obs_w)
        encoder = FrozenCNNViTEncoder(
            in_channels=img_shape[0], height=img_shape[1], width=img_shape[2],
            output_dim=int(encoder_dim), seed=int(encoder_seed),
        )
        input_dim = int(encoder_dim)
        ...
        print(
            f'CoGames CNN+ViT eval: mission={mission}  obs_shape={obs_shape}  '
            f'raw_input_dim={raw_input_dim}  obs_encoder=cnn_vit  '
            f'img_shape={img_shape}  encoder_dim={encoder_dim}  '
            f'encoder_seed={encoder_seed}  input_dim={input_dim}  '
            f'num_actions={num_actions}  num_agents={num_agents}  '
        ...
        for ep_idx in range(eval_episodes):
            ...
            for _agent_id in range(num_agents):
                ...
                policy_obj = CoGamesAgentPolicy(
                    ...
                    encoder=encoder, img_shape=img_shape,
                )
...
def main(argv):
    ...
    parser.add_argument(
        '--obs_encoder', type=str, default='cnn_vit',
        choices=['cnn_vit'],
        help='This variant always uses the frozen CNN+ViT encoder (a rasterised '
             '(num_features,H,W) image -> encoder_dim embedding). Flag kept for '
             'parity/logging with the other Hebbian evaluators.',
    )
    parser.add_argument('--encoder_dim', type=int, default=64,
                        help='Output dim of the frozen CNN+ViT encoder (= Hebbian '
                             'MLP input dim).')
    parser.add_argument('--encoder_seed', type=int, default=1337,
                        help='Seed for the frozen CNN+ViT weights; share across '
                             'runs => identical encoder.')
    parser.add_argument(
