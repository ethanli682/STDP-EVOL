...
def reconstruct_full_hebb_params(gene_record: dict, params_config: dict):
    """
    Reconstruct the full Hebbian and CNN evaluation payload from a gene record.

    This is a deterministic operation: W0 is generated from a fixed seed so no
    re-evaluation is needed. Used by saveFounder() to save a companion
    _full_params.pt.

    Args:
        gene_record: Gene record dict (as stored in history / bestChromosome).
                     Must have gene_record['param'] = decoded params dict containing
                     beta_X scalars and X_lY_u / X_lY_v arrays.
        params_config: Full YAML config dict.  Reads main.HebLoRA.{rank, w0_seed}.

    Returns:
        dict with:
            - 'hebb_coeffs': np.ndarray float32, shape (91328, 5)
            - 'cnn_params': np.ndarray float32, shape (1362,)
        This captures the full evolved evaluation payload for CarRacing.
    """
    import numpy as np

    # Architecture constants (must match policies.py)
    LAYER_SIZES    = [(648, 128), (128, 64), (64, 3)]
    N_COEFFS       = 5
    TOTAL_SYNAPSES = sum(d_in * d_out for d_in, d_out in LAYER_SIZES)   # 91328
    CNN_N_PARAMS   = 1362
    ...
    if 'cnn' not in gene_dict:
        raise ValueError("Missing gene key 'cnn'.")
    cnn_arr = np.asarray(gene_dict['cnn'], dtype=np.float32).ravel()
    if cnn_arr.size != CNN_N_PARAMS:
        raise ValueError(
            f"Gene key 'cnn' has {cnn_arr.size} values; expected {CNN_N_PARAMS}."
        )
...
def _run_task_impl(param, args):
    ...
    # =========================================================================
    # HebbianMetaLearning  –  CarRacing-v0, ABCD_lr (LoRA + beta over frozen W0)
    # =========================================================================
    #
    # For each coefficient family f in {A,B,C,D,lr} and each FC layer l:
    #   W_f,l = beta_f * W0_f,l + (U_f,l @ V_f,l)
    #
    # W0_f,l is deterministic frozen noise generated on-the-fly from:
    #   sigma(W0) = 1 / sqrt(d_in)
    #   RNG seed from YAML main.HebLoRA.w0_seed
    #
    # Evolved parameters are:
    #   beta_A, beta_B, beta_C, beta_D, beta_lr
    #   {A|B|C|D|lr}_l{1|2|3}_u and {A|B|C|D|lr}_l{1|2|3}_v
    # =========================================================================
    ...
    CNN_N_PARAMS   = 1362                               # conv1: 6x3x3x3=162, conv2: 8x6x5x5=1200
    COEFF_FAMILIES = ['A', 'B', 'C', 'D', 'lr']

    # --- Path configuration --------------------------------------------------
    # Adjust HEBB_EVAL_SCRIPT to match your repo layout.
    ...
    lora_rank_default = 8
    w0_seed_default = 12345
    lora_rank = lora_rank_default
    w0_seed = w0_seed_default
    eval_episodes = 3
    seed_base = 12345
    eval_render = False
    try:
        ...
        print(f'WARNING: failed to parse HebLoRA config from YAML ({_cfg_exc}); using defaults.', flush=True)
    ...
    cnn_arr = np.asarray(gene_dict.get('cnn', []), dtype=np.float32).ravel()
    if cnn_arr.size != CNN_N_PARAMS:
        raise ValueError(
    ...
    print(f'cnn_params_path={cnn_params_path}', flush=True)
...
def main(args):
    ...
    if len(params_config) == 0:
        msg = f"Loaded newMember for agent={args.agent_idx} is an empty list (file may be corrupt or was written with 0 candidates)"
