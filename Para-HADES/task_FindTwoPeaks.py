...
def run_task(param, args):
    ...
    # 1. Extract the 'TwoPeaks' parameter (the agent's position)
    # We look for 'TwoPeaks' explicitly, or fallback to the first key found
    source = list()
    if 'TwoPeaks' in param['param']:
        source = param['param']['TwoPeaks']
    else:
        for k in param['param'].keys():
            source = param['param'][k]
            break
            
    # Ensure source is a numpy array
    x = np.array(source, dtype=float).squeeze()
    
    # 2. Define the Two Peaks 
    # Since input dimension can vary, we define peaks as relative offsets or fixed points
    # Here we assume a symmetric setup common in this benchmark: 
    # Peak 1 is at -2.0, Peak 2 is at +2.0 (across all dimensions)
    center1 = np.full_like(x, -2.0)
    center2 = np.full_like(x, 2.0)

    # 3. Calculate Distance to centers
    # Using Euclidean distance squared
    dist1 = np.sum((x - center1) ** 2)
    dist2 = np.sum((x - center2) ** 2)

    # 4. Calculate Density (Fitness)
    # The goal is to MAXIMIZE this value
    density = 0.5 * np.exp(-dist1) + 0.5 * np.exp(-dist2)
    
    # Store raw metric
    outP['tests'].append(density)

    # 5. Set Fitness Score
    # Two Peaks is a MAXIMIZATION problem.
    # If evolutionTarget == -1 (Minimize mode), we return negative density (so minimizing it maximizes density).
    # If evolutionTarget != -1 (Maximize mode), we return positive density.
    if args.evolutionTarget == -1:
        outP['fitnessScore'] = -float(density)
    else:
        outP['fitnessScore'] = float(density)
