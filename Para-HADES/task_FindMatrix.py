...
def run_task(param, args):
    ...
    source = source.reshape(2,-1)
    source = source[0,:] * source[1,:].reshape(-1,1)
    ...
    for tar in range(len(target)):
        tmp = rmse( source, target[tar][0] * target[tar][1].reshape(-1,1) )
        tmp_targetr.append(target[tar][0].tolist())
        tmp_targetr[-1].extend(target[tar][1].tolist())
        outP['tests'].append(float(str(tmp)))

    # restore a flatten target for plotting
    target = tmp_targetr
