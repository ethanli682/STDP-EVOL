...
def run_task(param, args):
    ...
    # 1. Parse Input Agent Vector
    source = list()
    for k in param['param'].keys():
        source.append(param['param'][k])
    
    agent_vector = np.array(source, dtype=float).squeeze()
    dim = len(agent_vector)

    # 2. Define 5 Multimodal Targets
    
    # # Target 0: The "Valley of Zero"
    # target_A = np.zeros(dim)

    # Target 0: The "Randomness Seed 42"
    #save random state
    ...
    for target in targets:
        dist = np.linalg.norm(agent_vector - target)
        distances.append(float(dist))
        outP['tests'].append(float(dist))
        
    min_dist = min(distances)
    
    outP['train'] = [float(np.std(agent_vector)), float(np.mean(agent_vector))]

    if args.evolutionTarget == -1:
        outP['fitnessScore'] = float(min_dist) 
    else:
        outP['fitnessScore'] = 1.0 / (1.0 + float(min_dist))
