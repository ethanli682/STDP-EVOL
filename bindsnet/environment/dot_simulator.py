...
class DotSimulator:
    ...
    def reset(self):
        ...
        # # Assuming your grid dimension is accessible via self.dim (usually 28)
        # # We use np.random.randint to pick a random row and column.
        
        # target_r = np.random.randint(0, self.dim)
        # target_c = np.random.randint(0, self.dim)
        
        # # Initialize the target dot at the random coordinates
        # self.dots = [Dot(r=target_r, c=target_c)]
        
        # # Optional but recommended: Randomize the agent's (white dot) spawn too!
        # # This prevents the agent from just memorizing "move right 5 spaces from the center"
        # agent_r = np.random.randint(0, self.dim)
        # agent_c = np.random.randint(0, self.dim)
        
        # # Safeguard: Ensure the agent doesn't accidentally spawn exactly on top of the target
        # while agent_r == target_r and agent_c == target_c:
        #      agent_r = np.random.randint(0, self.dim)
        #      agent_c = np.random.randint(0, self.dim)

        # self.netDot = Dot(r=agent_r, c=agent_c)
