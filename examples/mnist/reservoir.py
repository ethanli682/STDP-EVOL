...
output = LIFNodes(n_neurons, thresh=-52 + np.random.randn(n_neurons).astype(float), traces=True)
network.add_layer(output, name="O")

from bindsnet.learning import PostPre, Hebbian

# Replace your current C1 and C2 definitions with this:
C1 = Connection(
    source=inpt, 
    target=output, 
    w=0.5 * torch.randn(inpt.n, output.n),
    update_rule=Hebbian, # Use Hebbian or PostPre (STDP)
    nu=(1e-4, 1e-4)      # nu[0] is for weakening, nu[1] is for strengthening
)
