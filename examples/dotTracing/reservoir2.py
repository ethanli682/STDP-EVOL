...
OUT_FILE_PATH = "weighted_reservoir25/"
...
def _view_index_to_offset(view_r):
    """For each input-neuron index k in getLocalView's flattened, center-excluded
    space, return its (drow, dcol) offset from the agent. Mirrors getLocalView's
    layout EXACTLY so receptive fields align with the real input indices."""
    side = 2 * view_r + 1
    center = view_r * side + view_r
    offs = []
    for k in range(side * side - 1):
        cell = k if k < center else k + 1     # re-insert the excised center gap
        pr, pc = divmod(cell, side)
        offs.append((pr - view_r, pc - view_r))
    return np.array(offs, dtype=np.float32)   # (inpt_n, 2)
 
 
def _polar_centers(view_r, n):
    """Tile n receptive-field centers on concentric rings (plus one at the agent),
    so every distance band -- near, mid, far -- has detector neurons. This is what
    lets the reservoir represent DISTANCE, not just direction."""
    rings = max(2, int(np.round(np.sqrt(n / np.pi))))
    radii = np.linspace(0.0, view_r, rings + 1)[1:]
    centers = [(0.0, 0.0)]                      # a near/center detector
    for R in radii:
        k = max(1, int(np.round(2 * np.pi * R / (view_r / rings))))
        for a in np.linspace(0, 2 * np.pi, k, endpoint=False):
            centers.append((R * np.sin(a), R * np.cos(a)))
    centers = np.array(centers[:n], dtype=np.float32)
    if len(centers) < n:                        # top up if rings gave too few
        idx = np.random.default_rng(0).integers(0, len(centers), n - len(centers))
        centers = np.vstack([centers, centers[idx]])
    return centers
 
 
def buildReceptiveFieldProjection(view_r, n_neurons, rf_radius=3.0, seed=0,
                                  device="cpu"):
    """Input -> reservoir projection where each reservoir neuron connects ONLY to
    input cells within rf_radius of its assigned center (tiled on concentric rings).
    Replaces the dense random projection so that neurons acquire LOCALIZED spatial
    receptive fields: center-tuned neurons fire for near prey, rim-tuned neurons for
    far prey. That radial, non-monotonic tuning is what makes distance linearly
    recoverable from the reservoir. Returns an (inpt_n, n_neurons) tensor."""
    rng = np.random.default_rng(seed)
    offs = _view_index_to_offset(view_r)         # (inpt_n, 2), matches getLocalView
    inpt_n = offs.shape[0]
    centers = _polar_centers(view_r, n_neurons)  # (n_neurons, 2)
    W = np.zeros((inpt_n, n_neurons), dtype=np.float32)
    for j in range(n_neurons):
        d = np.sqrt(((offs - centers[j]) ** 2).sum(1))
        in_field = d <= rf_radius
        if in_field.sum() == 0:                  # guarantee a non-empty field
            in_field = np.zeros(inpt_n, bool); in_field[np.argmin(d)] = True
        w = np.clip(rng.normal(1.0, 0.3, size=in_field.sum()), 0.1, None)
        W[in_field, j] = w
...
def main():
    ...
    network.add_layer(out, name="Output")
        ...
        # Input -> Middle: LOCALIZED RECEPTIVE FIELDS (replaces dense random projection).
    # Each reservoir neuron sees only a small patch of the egocentric view, with its
    # center tiled on concentric rings. Center-tuned neurons fire for NEAR prey,
    # rim-tuned neurons for FAR prey -> the reservoir can now represent distance,
    # not just direction. Scaled by 1/sqrt(mean-fan-in) so per-neuron drive stays
    # in the same range as before; input_gain then sets absolute excitability.
    input_gain = 6.0
    rf_radius = 3.0   # receptive-field radius in view cells; larger = more overlap
    W_rf = buildReceptiveFieldProjection(
        view_r, middle.n, rf_radius=rf_radius, seed=0, device=DEVICE
    )
    fan_in = (W_rf != 0).float().sum(0).mean().clamp(min=1.0)   # mean inputs/neuron
    W_rf = W_rf / fan_in.sqrt() * input_gain
    ...
    feat_weight_mid_out = Weight(
        name="weight_mid_out",
        value=torch.rand(middle.n, out.n, device=DEVICE) * 7.8, 
        learning_rule=MSTDPET,
        nu=[1e-2, 1e-2],
