...
def runSimulator(
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            spike_record[step % steps] = spikes[LAYER4].get("s").squeeze()
...
#multilayer feedforwaed WSNN
def main():
    # Build network.
    network = Network(dt=dt)

    inpt = Input(n=dim * dim, shape=[1, 1, 1, dim, dim], traces=True)
    middle1 = AdaptiveLIFNodes(n=neurons, traces=True, thresh=-52.0, theta_plus = 0.1, tc_theta=50.0)
    middle2 = AdaptiveLIFNodes(n=neurons, traces=True, thresh=-52.0, theta_plus = 0.1, tc_theta=50.0)
    out = LIFNodes(n=moveChoices, traces=True, thresh=-52.0, tc_decay=50.0)


    network.add_layer(inpt, name="Input")
    network.add_layer(middle1, name="Middle1")
    network.add_layer(middle2, name="Middle2")
    ...
    delay_in_mid1 = torch.rand((inpt.n, middle1.n), device=DEVICE)
    feat_delay_in_mid1 = Delay(
        name="delay_in_mid1",
        value=delay_in_mid1,
        max_delay=max_delay, # currently 32
    )
    feat_weight_in_mid1 = Weight(
        name="weight_in_mid1",
        value=torch.full((inpt.n, middle1.n), 1, device=DEVICE),
    )
    inpt_middle1 = MulticompartmentConnection(
        source=inpt, target=middle1,
        pipeline=[feat_delay_in_mid1, feat_weight_in_mid1],  # Delay MUST come first
        device=DEVICE,
    )

    # 3. Middle1 -> Middle1 LATERAL INHIBITION
    inhibition_strength1 = -2.5  # currently this could work
    w_recurrent1 = torch.ones(neurons, neurons, device=DEVICE) * inhibition_strength1
    w_recurrent1.fill_diagonal_(0.0)
    
    # no learning, a neuron inhibits all but itself
    feat_lat_inhibit1 = Weight(
        name="lat_inhibit",
        value=w_recurrent1,
        learning_rule=None
    )
    ...
    middle1_middle1 = MulticompartmentConnection(
        source=middle1,
        target=middle1,
        pipeline=[feat_lat_inhibit1],
        device=DEVICE
    )

    # middle1_middle1.delay = torch.ones(neurons, neurons, device=DEVICE)

    # delay_mid_out = torch.randint(0, max_delay + 1, (middle.n, out.n), device=DEVICE).float()
    delay_mid1_mid2 = torch.rand((middle1.n, middle2.n), device=DEVICE)
    feat_delay_mid1_mid2 = Delay(
        name="delay_mid1_mid2",
        value=delay_mid1_mid2,
        max_delay=max_delay, # currently 32
        nu=[1e-2, 1e-2],
        learning_rule=MSTDPET,
    )
    feat_weight_mid1_mid2 = Weight(
        name="weight_mid1_mid2",
        value=torch.ones(middle1.n, middle2.n, device=DEVICE),
        # INCRESE this if output is not spiking enough
    )
    middle1_middle2 = MulticompartmentConnection(
        source=middle1, target=middle2,
        pipeline=[feat_delay_mid1_mid2, feat_weight_mid1_mid2],  
        device=DEVICE,
    )

# 3. Middle2 -> Middle2 LATERAL INHIBITION
    inhibition_strength2 = -1.5  # currently this could work
    w_recurrent2 = torch.ones(neurons, neurons, device=DEVICE) * inhibition_strength2
    w_recurrent2.fill_diagonal_(0.0)
    
    # no learning, a neuron inhibits all but itself
    feat_lat_inhibit2 = Weight(
        name="lat_inhibit",
        value=w_recurrent2,
        learning_rule=None
    )

    middle2_middle2 = MulticompartmentConnection(
        source=middle2,
        target=middle2,
        pipeline=[feat_lat_inhibit2],
        device=DEVICE
    )



    delay_mid2_out = torch.rand((middle2.n, out.n), device=DEVICE)
    feat_delay_mid2_out = Delay(
        name="delay_mid2_out",
        value=delay_mid2_out,
        max_delay=max_delay, # currently 32
        nu=[1e-2, 1e-2],
        learning_rule=MSTDPET,
    )
    feat_weight_mid2_out = Weight(
        name="weight_mid2_out",
        value=torch.ones(middle2.n, out.n, device=DEVICE),
        # INCRESE this if output is not spiking enough
    )
    middle2_out = MulticompartmentConnection(
        source=middle2, target=out,
        pipeline=[feat_delay_mid2_out, feat_weight_mid2_out],  
        device=DEVICE,
    )

    network.add_connection(inpt_middle1, source="Input", target="Middle1")
    network.add_connection(middle1_middle1, source="Middle1", target="Middle1")
    network.add_connection(middle1_middle2, source="Middle1", target="Middle2")
    network.add_connection(middle2_middle2, source="Middle2", target="Middle2")
    network.add_connection(middle2_out, source="Middle2", target="Output")

    network.to(DEVICE)

    delay_features = {"inpt_middle1": feat_delay_in_mid1, "middle1_middle2": feat_delay_mid1_mid2}
