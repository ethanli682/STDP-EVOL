...
def main():
    ...
    # Convert to Adaptive LIF Nodes to prevent oversaturation in weightless setup
    middle = AdaptiveLIFNodes(
        n=neurons, 
        traces=True,
        thresh=-52.0,      # Base threshold (theta_0)
        theta_plus=1.9,    # Voltage increase after incoming signal
        tc_theta=0.1       # Decay rate
    )

    out = AdaptiveLIFNodes(
        n=moveChoices, 
        refrac=0, 
        traces=True,
        thresh=-52.0,
    ...
    # ----------------------------------------------------
    # CONNECTION 1: Input -> Middle (Forward, Static Delays)
    # ----------------------------------------------------
    # Weights frozen to 1.0, data is encoded in the delay initialization
    delay_in_mid = torch.randint(0, 32, (inpt.n, middle.n), device=DEVICE)
    feat_in_mid = Weight(
        name="feat_in_mid",
        value=torch.ones(inpt.n, middle.n, device=DEVICE),
    ...
    inpt_middle = MulticompartmentConnection(
        source=inpt,
        target=middle,
        pipeline=[feat_in_mid],
        delay=delay_in_mid,
        device=DEVICE
    )

    # ----------------------------------------------------
    # CONNECTION 2: Middle -> Output (Delay STDP)
    # # ----------------------------------------------------

    # delay_mid_out = torch.randint(0, 32, (middle.n, out.n), device=DEVICE)
    delay_mid_out = torch.rand((inpt.n, middle.n), device=DEVICE)
    feat_mid_out = Weight(
        name="feat_mid_out",
        value=torch.ones(middle.n, out.n, device=DEVICE),
        learning_rule=None
    )
    ...
    middle_out = MulticompartmentConnection(
        source=middle,
        target=out,
        pipeline=[feat_mid_out],
        device=DEVICE
    )
    
    # --- FIX: Explicitly bind the delay tensor so the learning rule and network can see it ---
    middle_out.delay = delay_mid_out
    
    # Manually instantiate and bind the learning rule
    middle_out.learning_rule = ModulatedDelaySTDP(
        connection=middle_out,
        nu=[1e-2, 1e-2],   
        tc_e=1000.0,       
        max_delay=32
    )

    # -500 inhibition to all other neurons
    
    # 1. Define a massive negative voltage to guarantee suppression
    inhibition_strength = -500.0  
    
    # 2. Create an all-to-all matrix of inhibitory weights
    w_recurrent = torch.ones(middle.n, middle.n, device=DEVICE) * inhibition_strength
    
    # 3. Remove self-inhibition (neurons should not silence themselves)
    w_recurrent.fill_diagonal_(0.0)
    
    feat_recurrent = Weight(
        name="feat_recurrent",
        value=w_recurrent,
        range=(inhibition_strength, 0.0) # Lock weights to negative values
    )
    
    # 4. Use a minimal delay of 1 timestep so the network computes sequentially
    # rather than getting caught in a cyclical calculation error
    delay_recurrent = torch.ones(middle.n, middle.n, device=DEVICE, dtype=torch.long)
    
    recurrent = MulticompartmentConnection(
        source=middle,
        target=middle,
        pipeline=[feat_recurrent],
        delay=delay_recurrent,
        device=DEVICE
    )
