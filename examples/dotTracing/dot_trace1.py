...
# 1. Route outputs to your scratch space to avoid home directory quota issues
OUT_FILE_PATH = "/cluster/scratch/eli08/bindsNet/dot_tracing_outputs/"

# 2. Force the program to create the directory if it doesn't exist
os.makedirs(OUT_FILE_PATH, exist_ok=True)

def genFileName(ftype, suffix=""):
    """
    Generates output file names using the Slurm Job ID if available, 
    falling back to system time for local debugging.
    """
    # Grab the Slurm Job ID directly from the HPC environment variables
    job_id = os.environ.get("SLURM_JOB_ID")
    
    if job_id:
        uid = f"job{job_id}"
    else:
        # Fallback to your original time-based naming if running off-cluster
        sysTime = time.time()
        uid = "s" + str(int(1e10 * (sysTime - 1e6 * (sysTime // 1e6))))

    # Safely join the paths
    return os.path.join(OUT_FILE_PATH, f"{ftype}_{uid}_{suffix}.csv")
...
def main():
    ...
    middle_out = Connection(
        ...
        update_rule=MSTDPET,
        nu=1e-2,  # learning rate
        norm=0.03 * middle.n,  # normalization
        tc_e_trace=90.0,
    )

    w_recurrent = torch.rand(middle.n, middle.n, device=DEVICE) * 0.2
    
    # Calculate how many neurons should be inhibitory (20% of 100 = 20 neurons)
    n_inhibitory = int(0.2 * middle.n)
    
    # Randomly select which neuron indices will be the inhibitory population
    inhibitory_indices = torch.randperm(middle.n)[:n_inhibitory]
    
    # Flip the signs of entire rows for those indices. 
    # This ensures that if a neuron is inhibitory, ALL its outgoing weights are negative.
    w_recurrent[inhibitory_indices, :] = -w_recurrent[inhibitory_indices, :]

    # -------------------------------------------------------------------------
    # 2. Instantiate the Recurrent Connection with the custom weights
    # -------------------------------------------------------------------------
    recurrent = Connection(
        source=middle,
        target=middle,
        w=w_recurrent,          # Pass your pre-configured E/I weight matrix
        wmin=-1.0,              # CRITICAL: Lower wmin to allow negative weights!
